...
cases |> 
  filter(state == "California") |> # filter state is CA
  mutate(date = date(date), # change these 3 variables into date time format
         month = month(date, label = FALSE, abbr = TRUE),
         year = year(date)) |> 
  unite("month_year", month: year) |> # combine month and year into one variable
  group_by(month_year)|> # group by month_year
  summarise(sum_daily = sum(cases_daily))|> # get the total cases for each month
  arrange(desc(sum_daily)) |># arrange so we can see ehich month had the most cases
  ggplot(aes(x = sum_daily, y = fct_reorder(month_year, sum_daily))) + # plot the monthly cases totals, and order them to be descending
  geom_col()
...
> January 2022 had the most COVID-19 cases between January 2020 and March 2023
...
cases |> 
  ...
  mutate(date = date(date), # change these 3 variables into date time format
         month = month(date, label = FALSE, abbr = TRUE),
         year = year(date)) |> 
  unite("month_year", year:month, sep = "-") |> # combine month and year into one variable
  filter(str_sub(month_year, start = 1, end = 4) == "2021") |> #select the first 4 letters in the string
  group_by(month_year)|> # group by month_year
  summarise(sum_daily = sum(cases_daily))|> # get the total cases for each month
  arrange(desc(sum_daily)) |># arrange so we can see ehich month had the most cases
  ggplot(aes(x = sum_daily, y = fct_reorder(month_year, sum_daily))) + # plot the monthly cases totals, and order them to be descending
...
cases |> 
  ...
  mutate(date = date(date), # change these 3 variables into date time format
         ...
         year = year(date)) |> 
    unite("month_year", year:month, sep = "-") |> # combine month and year into one variable
  group_by(month_year)|> # group by year
  mutate(death_ratio = deaths/cases)|> # get the total cases for each month
  ggplot(aes(x = date, y = death_ratio)) + # plot the monthly cases totals, and order them to be descending
  geom_point()+
  labs(title = "Covid 19 Deaths to Cases Ratio 2020-2023",
       subtitle = "Research question: What year between 2020 and 2023 had the highest cases to death ratio in California?",
       x = "Date",
       y = "Deaths to Case Ratio")+
  theme_light()
ggsave("death_to_cases_plot.png")
```
